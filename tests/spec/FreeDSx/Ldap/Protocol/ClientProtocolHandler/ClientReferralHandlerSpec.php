...
class ClientReferralHandlerSpec extends ObjectBehavior
{
    ...
    function it_should_throw_an_exception_on_referrals(LdapQueue $queue)
    {
    ...
    function it_should_follow_referrals_with_a_referral_chaser_if_specified(ReferralChaserInterface $chaser, LdapQueue $queue, LdapClient $ldapClient)
    {
    ...
    function it_should_throw_an_exception_if_the_referral_limit_is_reached(ReferralChaserInterface $chaser, LdapQueue $queue)
    {
    ...
    function it_should_throw_an_exception_if_all_referrals_have_been_tried_and_follow_is_specified(ReferralChaserInterface $chaser, LdapQueue $queue)
    {
    ...
    function it_should_continue_to_the_next_referral_if_a_connection_exception_is_thrown(ReferralChaserInterface $chaser, LdapQueue $queue, LdapClient $ldapClient)
    {
        $chaser->client(Argument::any())->willReturn($ldapClient);
        $bind = new SimpleBindRequest('foo', 'bar');
    ...
    function it_should_continue_to_the_next_referral_if_an_operation_exception_with_a_referral_result_code_is_thrown(ReferralChaserInterface $chaser, LdapQueue $queue, LdapClient $ldapClient)
    {
        $chaser->client(Argument::any())->willReturn($ldapClient);
        $bind = new SimpleBindRequest('foo', 'bar');
        $chaser->chase(Argument::any(), Argument::any(), Argument::any())->willReturn($bind);
    ...
    function it_should_not_bind_on_the_referral_client_initially_if_the_referral_is_for_a_bind_request(ReferralChaserInterface $chaser, LdapQueue $queue, LdapClient $ldapClient)
    {
        $chaser->client(Argument::any())->willReturn($ldapClient);
