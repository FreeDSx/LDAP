...
class LdapUrlExtensionSpec extends ObjectBehavior
{
    ...
    {
        $this::parse('foo')
            ->shouldBeLike(new LdapUrlExtension('foo'));
    }
    ...
    {
        $this::parse('!foo')
            ->shouldBeLike(new LdapUrlExtension(
                'foo',
                null,
                true
            ));
    ...
    {
        $this::parse('e-bindname=cn=Manager%2cdc=example%2cdc=com')
            ->shouldBeLike(new LdapUrlExtension(
                'e-bindname',
    ...
    public function it_should_generate_a_string_extension_and_encode_it_if_needed(): void
    {
        ...
        $this->toString()
            ->shouldBeEqualTo('e-bindname=cn=Manager%2cdc=example%2cdc=com');
    }
}
