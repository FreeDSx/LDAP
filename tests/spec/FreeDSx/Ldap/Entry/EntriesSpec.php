...
class EntriesSpec extends ObjectBehavior
{
    ...
    public function it_should_get_the_first_entry()
    {
        $this->first()->shouldBeLike(new Entry('foo'));
    }

    public function it_should_return_null_if_the_first_entry_does_not_exist()
    {
    ...
    public function it_should_get_the_last_entry()
    {
        $this->last()->shouldBeLike(new Entry('bar'));
    }

    public function it_should_return_null_if_the_last_entry_does_not_exist()
    {
        $this->beConstructedWith(...[]);
    ...
    public function it_should_add_entries()
    {
        $this->add(new Entry('cn=new'), new Entry('cn=another'));
        $this->count()->shouldBeEqualTo(4);
    }

    public function it_should_remove_entries()
    {
    ...
    public function it_should_not_remove_entries_if_they_dont_exist()
    {
        $this->remove(new Entry('cn=meh'));
        $this->count()->shouldBeEqualTo(2);
    }

    public function it_should_check_if_an_entry_object_is_in_the_collection()
    {
    ...
    public function it_should_check_if_an_entry_is_in_the_collection_by_dn()
    {
        $this->has('foo')->shouldBeEqualTo(true);
        $this->has('cn=meh')->shouldBeEqualTo(false);
    }

    public function it_should_get_an_entry_by_dn()
    {
        $this->get('foo')->shouldBeLike(new Entry('foo'));
    }

    public function it_should_return_null_when_trying_to_get_an_entry_that_doesnt_exist()
    {
        $this->get('meh')->shouldBeNull();
    }

    public function it_should_get_the_array_of_entries()
    {
