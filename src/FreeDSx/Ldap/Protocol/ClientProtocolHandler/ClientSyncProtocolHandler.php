...
class ClientSyncProtocolHandler
{
    ...
    public function handle() : ?LdapMessageResponse
    {
        ...
        return new LdapMessageResponse(
            ...
            new SyncResponse($this->lastResponse->getResponse(), $cookie, $present, $deleted, $initial, $updates),
    ...
    private function initialContent() : array
    {
        ...
        foreach ($this->queue->getMessages($this->messageTo->getMessageId()) as $message) {
            ...
            if ($response instanceof SearchResultEntry) {
                $syncResult = new SyncResult($response->getEntry(), $message->controls()->get(Control::OID_SYNC_STATE));
            } elseif ($response instanceof SearchResultReference) {
                $syncResult = new SyncResult($response->getReferrals(), $message->controls()->get(Control::OID_SYNC_STATE));
        ...
        do {
            ...
            foreach ($this->queue->getMessages($this->messageTo->getMessageId()) as $message) {
                ...
                if ($response instanceof SyncRefreshDelete) {
                    ...
                    if ($response instanceof SearchResultEntry) {
                        $syncResults[] = new SyncResult($response->getEntry(), $message->controls()->get(Control::OID_SYNC_STATE));
                    } elseif ($response instanceof SearchResultReference) {
                        $syncResults[] = new SyncResult($response->getReferrals(), $message->controls()->get(Control::OID_SYNC_STATE));
                    } elseif ($response instanceof SearchResultDone) {
                        $isDone = true;
                        break;
                    }
